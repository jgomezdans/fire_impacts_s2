...
from .s2_reader import S2File, NoS2File, search_s2_tiles, S2FileGEE
...
class Observations(object):
    def __init__(self, pre_fire, post_fire, temp_folder=None):
        ...
        try:
            ...
            self.post_fire = LC8File(
                post_fire,
                master_file=self.pre_fire.surface_reflectance.as_posix(),
                temp_folder=temp_folder,
            )
            log.info("Spectral setup for Landsat8")
            self.sensor = "LC8"
            self.wavelengths = np.array([480.0, 560.0, 655.0, 865.0, 1610.0, 2200])
            self.bu = np.array([8.5, 5.4, 4.0, 2.6, 1.1, 3.6]) / 1000.0
            ...
            try:
                ...
                self.wavelengths = np.array(
                    [490.0, 560.0, 665.0, 705, 740.0, 783, 865.0, 1610.0, 2190]
                )
...
class GEEObservations(object):
    def __init__(self, pre_fire, post_fire, temp_folder=None):

        self.pre_fire = S2FileGEE(Path(pre_fire))
        self.post_fire = S2FileGEE(Path(post_fire))
        ...
        self.rho_pre_prefix = "pre"
        self.rho_post_prefix = "post"
...
class FireImpacts(object):
    def __init__(
        self, observations, output_dir=".", quantise=False, user_a0=None, user_a1=None
    ):
        if (user_a0 is not None and user_a1 is None) or (
            user_a0 is None and user_a1 is not None
        ):
    ...
    def launch_processor(self):
        ...
        try:
            mask = self.observations.pre_fire.mask * self.observations.post_fire.mask
        ...
        for (ds_config, this_X, this_Y, nx_valid, ny_valid, data) in extract_chunks(
            the_fnames
        ):
            if first:
                self.create_output(
            ...
            if mask is not None:
                M = mask[this_Y : (this_Y + ny_valid), this_X : (this_X + nx_valid)]
            else:
                M = np.ones_like(data[0][0], dtype=bool)
            if M.sum() == 0:
            ...
            if self.user_a0 is None or self.user_a1 is None:
                xfcc, a0, a1, rmse = invert_spectral_mixture_model(
                    rho_pre, rho_post, M, self.observations.bu, self.observations.lk
                )
            else:
            ...
            if self.save_quantised:
                xfcc = np.where(
                    np.logical_and(xfcc > 1e-3, xfcc <= 1.2), xfcc * 100, 255
                ).astype(np.uint8)
                a0 = np.where(
                    np.logical_and(a0 > 1e-3, a1 <= 0.2), a0 * 500, 255
                ).astype(np.uint8)
                a1 = np.where(
                    np.logical_and(a1 > 1e-3, a1 <= 0.5), a1 * 400, 255
                ).astype(np.uint8)

            self.ds_params.GetRasterBand(1).WriteArray(xfcc, xoff=this_X, yoff=this_Y)
            self.ds_params.GetRasterBand(2).WriteArray(a0, xoff=this_X, yoff=this_Y)
            self.ds_params.GetRasterBand(3).WriteArray(a1, xoff=this_X, yoff=this_Y)
            self.ds_rmse.GetRasterBand(1).WriteArray(rmse, xoff=this_X, yoff=this_Y)
            log.info("Burp!")
        self.ds_burn = None
        self.ds_fwd = None
        self.ds_params = None
    ...
    def create_output(
        ...
        if self.save_quantised:
            self.ds_params = drv.Create(
                output_fname, Nx, Ny, 3, gdal.GDT_Byte, options=gdal_opts
            )
        else:
            self.ds_params = drv.Create(
                output_fname, Nx, Ny, 3, gdal.GDT_Float32, options=gdal_opts
            )
        ...
        self.ds_rmse = drv.Create(
            output_fname, Nx, Ny, 1, gdal.GDT_Float32, options=gdal_opts
        )
...
if __name__ == "__main__":
    ...
    fire_imp = FireImpacts(GEEObservations(s2_pref, s2_postf))
    fire_imp.launch_processor()

# if __name__ == "__main__":
#     granules = search_s2_tiles("/data/selene/ucfajlg/fcc_sentinel2/Alberta/", "T12VVH")
