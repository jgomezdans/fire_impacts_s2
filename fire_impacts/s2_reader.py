...
def search_s2_tiles(s2_path, tile):
    ...
    for fich in s2_path.glob(f"**/*{tile}*.SAFE"):
        if fich.is_dir():
            granules[
                datetime.datetime.strptime(fich.name.split("_")[2], "%Y%m%dT%H%M%S")
            ] = fich
    return granules
...
class S2FileVintageFormat(object):
    def __init__(self, granule_path):
        ...
        self.acq_time = datetime.datetime.strptime(
            self.granule_path.name.split("_")[-4], "%Y%m%dT%H%M%S"
        )
        ...
        for fich in img_path.glob("*_20m.jp2"):
            if not (
                (fich.as_posix().find("AOT") >= 0)
                or (fich.as_posix().find("WVP") >= 0)
                or (fich.as_posix().find("VIS") >= 0)
            ):
...
class S2FileNewFormat(object):
    def __init__(self, granule_path):
        ...
        if not self.granule_path.exists():
            raise IOError(f"{self.granule_path.name} does not exist!")
        self.acq_time = datetime.datetime.strptime(
            granule_path[: granule_path.find(".SAFE")].split("/")[-1].split("_")[2],
            "%Y%m%dT%H%M%S",
        )
        self.pathrow = (
            granule_path[: granule_path.find(".SAFE")].split("/")[-1].split("_")[-2]
        ...
        for fich in img_path.glob("*_20m.jp2"):
            if not (
                ...
                or (fich.as_posix().find("WVP") >= 0)
                or (fich.as_posix().find("VIS") >= 0)
                or (fich.as_posix().find("SCL") >= 0)
                or (fich.as_posix().find("TCI") >= 0)
            ):
...
class S2FileGEE(object):
    def __init__(self, granule_path):
        ...
        log.debug("Dealing with S2 GEE file")
        self.pathrow = self.granule_path.stem

        log.info(f"S2 tile code is {self.pathrow}")
        surf_refl = [None for i in range(len(S2_BANDS))]

        surf_reflectance_output = self.granule_path
        # R = gdal.Info(self.granule_path.as_posix())
        # ul_text = [r for r in R.split("\n") if r.find("Upper Left") >= 0]
        # lr_text = [r for r in R.split("\n") if r.find("Lower Right") >= 0]
        # lr_x, lr_y = [float(x.strip(",)")) for x in lr_text[0].split()[3:5]]
        # ul_x, ul_y = [float(x.strip(",)")) for x in ul_text[0].split()[3:5]]
...
if __name__ == "__main__":
    ...
    ff = S2FileSIAC(
        Path(
            "/data/selene/ucfajlg/Chile/S2A_MSIL1C_20170218T143751_N0204_R096_T18HYF_20170218T145150.SAFE"
        )
    )
